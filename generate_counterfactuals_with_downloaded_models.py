#!/usr/bin/env python3
"""
Generate counterfactuals using downloaded DenseFlow models.
Supports CIFAR-10, CIFAR-100, CUB200, Flowers, and Pets datasets.
"""
...
class DenseFlowCounterfactualGenerator:
    """Counterfactual generator using downloaded DenseFlow models."""
    ...
    def generate_counterfactual(self, source_image, target_class, alpha=0.5):
        """Generate counterfactual using the loaded model."""
        with torch.no_grad():
            try:
                ...
                # Get target prototype (simplified - using class mean in latent space)
                target_proto = self._get_class_prototype(target_class, z_flat.shape[1])
                ...
                # Decode back to image using the flow
                try:
                    ...
                    try:
                        ...
                        print("Flow decoding failed, using direct interpolation")
                        # Fallback to direct interpolation
                        cf_image = (1 - alpha) * source_image + alpha * torch.randn_like(source_image)
                ...
                traceback.print_exc()
                # Fallback to direct interpolation
                return (1 - alpha) * source_image + alpha * torch.randn_like(source_image)
    
    def _get_class_prototype(self, class_idx, feature_dim):
        """Get prototype for a specific class in latent space."""
        # For now, use a simple approach - you might want to improve this
        # by computing actual class means in latent space
...
def main():
    parser = argparse.ArgumentParser(description='Generate Counterfactuals with Downloaded DenseFlow Models')
    ...
    parser.add_argument('--output_dir', default='./counterfactual_results_downloaded', help='Output directory')
    ...
    try:
        ...
        print(f"Initializing counterfactual generator for {args.dataset}...")
        generator = DenseFlowCounterfactualGenerator(args.model_dir, args.dataset, device)
        ...
        print(f"\n{'='*60}")
        print(f"GENERATING COUNTERFACTUAL EXPLANATIONS FOR {args.dataset.upper()}")
        print(f"{'='*60}")
